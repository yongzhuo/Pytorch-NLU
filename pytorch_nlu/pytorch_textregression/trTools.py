...
def mertics_multilabel_confusion_matrix(y_true: Any, y_pred: Any):
    ...
    # true_and_pred = y_true.astype(np.float16) * y_pred.astype(np.float16)  # 每个元素相乘, 0,1, 可替换为压缩矩阵"csr"的形式
    true_and_pred = y_true.astype(np.int32) * y_pred.astype(np.int32)  # 每个元素相乘, 0,1, 可替换为压缩矩阵"csr"的形式
...
def sklearn_regression_mertics(y_true: List, y_pred: List, eval_type="MSE"):
    """ sklearn, 先multi-class, 然后multi-label
    calculate confusion_matrix, 计算混淆矩阵
    Args:
        y_true: list, label of really true, eg. [0.2, 0.1]
        y_pred: list, label of model predict, eg. [2, "FALSE"]
        eval_type: string, type of regression mertics, eg. ["MSE", "MSLE", "MAE"]
    Returns:
        metrics(confusion)
    """
    from sklearn.metrics import mean_squared_error, mean_squared_log_error, mean_absolute_error
    try:
        if eval_type.upper() == "MSLE":
            mertics_number = mean_squared_log_error(y_true, y_pred)
        elif eval_type.upper() == "MAE":
            mertics_number = mean_absolute_error(y_true, y_pred)
        else:
            mertics_number = mean_squared_error(y_true, y_pred)
    except Exception as e:
        logging.info(str(e))
        mertics_number = -1
    return {"micro_avg": {"mse": mertics_number}}
...
def get_logger(log_dir: str, back_count: int=32, logger_name: str="pytorch_nlp_tc"):
    ...
    #                                maxBytes=back_count * 1024 * 1024,
    #                                backupCount=back_count, encoding="utf-8")
    fHandler = TimedRotatingFileHandler(log_name_day, when="MIDNIGHT", #'D',
                                        interval=1, backupCount=back_count,
...
def find_diff_pos(sent1, sent2) -> List:
    """
...
if __name__ == '__main__':
    ...
    sent1, sent2 = '春天来了，越来越来暖和了。', '春天来了，天气越来越暖和了。'
    resp = find_diff_pos(sent1, sent2)
    print(resp)
